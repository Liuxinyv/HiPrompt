    ...
    ####################################################### Phase Upscaling #####################################################
        ...
        for current_scale_num in range(starting_scale, scale_num + 1):
            ...
            if share:
                prompts_part = []
                prompts_images=[]
                views_local,num_blocks_height,num_blocks_width = self.get_views(current_height, current_width, stride=stride, window_size=self.default_sample_size, random_jitter=True,seed=seed)#9*4
                ...
                latents_l = F.pad(latents_share, (jitter_range, jitter_range, jitter_range, jitter_range), 'constant', 0)#[1, 4, 288, 288]
                
                for i in range(len(part_coordinates)):
                    h_start, h_end, w_start, w_end = part_coordinates[i]
                    latents_part_i = latents_share[:, :, h_start:h_end, w_start:w_end]
                ...
                (
                    prompt_embed,#[9, 120, 4096],[1, 77, 2048]
                    negative_prompt_embed,#[1, 77, 2048]
                    prompt_attn_mask,#[9, 120],[1, 1280]
                    negative_prompt_attn_mask,#[9, 120],[1, 1280]
                ) = self.encode_prompt(
                    prompt=prompts_part,
                    prompt_2=prompt_2,
                    device=device,
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    if i>early_start:
                        ...
                        views,_,_ = self.get_views(current_height, current_width, stride=stride, window_size=self.unet.config.sample_size, random_jitter=True,seed=seed)
                        ...
                        for j, batch_view in enumerate(views_batch):
                            ...
                            if do_classifier_free_guidance:
                                ...
                                if noise_decom:
                                    ...
                                    inverted_preds = []
                                    for pred, view_fact in zip(noise_pred_text, views_fact*vb_size):
                                        inverted_pred = view_fact.inverse_view(pred)
                                        inverted_preds.append(inverted_pred)
