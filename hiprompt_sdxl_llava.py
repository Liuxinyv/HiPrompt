...
class HiPromptSDXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    """
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
                ...
                pooled_prompt_embeds = prompt_embeds[0]#([9, 77, 768])
                prompt_embeds = prompt_embeds.hidden_states[-2]#([9, 77, 768]),([1, 77, 1280])
    ...
    def filter_ngrams(self,ngrams_list):
        nltk.download('stopwords')
        stop_words = set(stopwords.words('english'))
        filtered_ngrams = []
        pattern = re.compile(r'^\d')
        for ngram in ngrams_list:
            if ngram[0].lower() in stop_words or ngram[0].lower() in {'of', 'on', 'in','over','with','forward'}:
                continue
            if ngram[-1].lower() in stop_words or ngram[-1].lower() in {'a', 'the', 'to', 'on','at'}:
                continue
            if pattern.match(ngram[0].lower()):
                continue
            filtered_ngrams.append(ngram)
        return filtered_ngrams
    
    def ngrams_prepare(self,ngrams_list):
        device = self._execution_device
        grams_prompt_embeds = []
        # for prompt, tokenizer, text_encoder in zip(ngrams_list, self.tokenizer, self.text_encoder):
        for prompt in ngrams_list:
            if len(prompt)!=1:
                prompt = [', '.join(prompt)]
            else:
                prompt=list(prompt)
            ...
            grams_prompt_embeds.append(prompt_embeds)

        grams_prompt_embeds = torch.concat(grams_prompt_embeds, dim=0)
        return grams_prompt_embeds
    ...
    def check_inputs(
        ...
        if num_images_per_prompt != 1:
            warnings.warn("num_images_per_prompt != 1 is not supported by HiPrompt and will be ignored.")
            num_images_per_prompt = 1

    # Copied from diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion.StableDiffusionPipeline.prepare_latents
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None,seed=3407):
    ...
    def get_views(self, height, width, window_size=128, stride=64, random_jitter=False,seed=3407):
        # Here, we define the mappings F_i (see Eq. 7 in the MultiDiffusion paper https://arxiv.org/abs/2302.08113)
        # if panorama's height/width < window_size, num_blocks of height/width should return 1
        random.seed(seed)
        height //= self.vae_scale_factor #256
        width //= self.vae_scale_factor
        num_blocks_height = int((height - window_size) / stride - 1e-6) + 2 if height > window_size else 1 #8
    ...
    def tiled_decode(self, latents, current_height, current_width,seed=3407):
        sample_size = self.unet.config.sample_size #128
        core_size = self.unet.config.sample_size // 4 #32
        core_stride = core_size #32
        pad_size = self.unet.config.sample_size // 8 * 3 #48
        ...
        if self.lowvram:
            ...
            pad_size = core_size
                                #2048
        views,_,_ = self.get_views(current_height, current_width, stride=core_stride, window_size=core_size,seed=seed) #64,144
        print(views)
        views_batch = [views[i : i + decoder_view_batch_size] for i in range(0, len(views), decoder_view_batch_size)]
        latents_ = F.pad(latents, (pad_size, pad_size, pad_size, pad_size), 'constant', 0) #[1, 4, 352, 352],[1, 4, 480, 480]
        image = torch.zeros(latents.size(0), 3, current_height, current_width).to(latents.device) #[1, 3, 2048, 2048],[1, 3, 3072, 3072]
        count = torch.zeros_like(image).to(latents.device)#[1, 3, 2048, 2048]
        # get the latents corresponding to the current view coordinates
        with self.progress_bar(total=len(views_batch)) as progress_bar:
            coors=[]
            for j, batch_view in enumerate(views_batch):
                vb_size = len(batch_view) #1
                latents_for_view = torch.cat(
                    [
                        latents_[:, :, h_start:h_end+pad_size*2, w_start:w_end+pad_size*2]
                        for h_start, h_end, w_start, w_end in batch_view
                    ]
                ).to(self.vae.device) #[1, 4, 128, 128]
                image_patch = self.vae.decode(latents_for_view / self.vae.config.scaling_factor, return_dict=False)[0] #[1, 3, 1024, 1024]
                ...
                p_h_start, p_h_end, p_w_start, p_w_end = pad_size * self.vae_scale_factor, image_patch.size(2) - pad_size * self.vae_scale_factor, pad_size * self.vae_scale_factor, image_patch.size(3) - pad_size * self.vae_scale_factor #(0, 256, 0, 256)
                coors.append((p_h_start//8, p_h_end//8, p_w_start//8, p_w_end//8))
        ...
        return image
    def sync_tiled_decode_scale(self, z: torch.FloatTensor, current_height=None, current_width=None):
        """
        ...
        for i in range(0, z.shape[2], overlap_size):
            for j in range(0, z.shape[3], overlap_size):
                # print(i,j,"i_1,j_1")
                # print(i,i + tile_latent_min_size,j,j + tile_latent_min_size)
                tile = z[:, :, i:i + tile_latent_min_size, j:j + tile_latent_min_size] #[1, 4, 128, 128]
                tile = self.vae.decode(tile / self.vae.config.scaling_factor, return_dict=False)[0]#([1, 3, 1024, 1024])
                tiles.append(tile)
    ...
    def __call__(
        ...
        image_enc: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        image_enc_2: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        scale: bool = False,
        sample_path: str = None,
        view_args: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        views_type: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        ...
        Args:
            ...
            image_lr (`torch.FloatTensor`, *optional*, , defaults to None):
                Low-resolution image input for upscaling. If provided, HiPrompt will encode it as the initial latent representation.
            ...
            cosine_scale_3 (`float`, defaults to 1):
                Control the strength of the gaussion filter. 
    ...
    ###################################################### Phase Initialization ########################################################
        ...
        if not output_type == "latent":
            ...
            if self.lowvram and multi_decoder:
                ...
                image = self.tiled_decode(latents, current_width_height, current_width_height,seed=seed)
            else:
                image = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
            # cast back to fp16 if needed
            if needs_upcasting:
                self.vae.to(dtype=torch.float16)
        latents_llava = latents.clone()
    ...
    ####################################################### Phase Upscaling #####################################################
        ...
        for current_scale_num in range(starting_scale, scale_num + 1):
            ...
            latents = noise_latents[0]#([1, 4, 256, 256])
            if llava:
                prompts_part = []
                views_local,num_blocks_height,num_blocks_width = self.get_views(current_height, current_width, stride=stride, window_size=self.default_sample_size, random_jitter=True)#9*4
                part_coordinates = list(map(lambda x: (math.ceil(x[0] // current_scale_num), math.ceil(x[1] // current_scale_num), math.ceil(x[2] // current_scale_num), math.ceil(x[3] // current_scale_num)), views_local))
                jitter_range = int((self.default_sample_size - stride) // 4 /current_scale_num )#16
                latents_l = F.pad(latents_llava, (jitter_range, jitter_range, jitter_range, jitter_range), 'constant', 0)#[1, 4, 288, 288]

                for i in range(len(part_coordinates)):
                    h_start, h_end, w_start, w_end = part_coordinates[i]
                    latents_part_i = latents_l[:, :, h_start:h_end, w_start:w_end]
                    image_part = self.vae.decode(latents_part_i / self.vae.config.scaling_factor, return_dict=False)[0]
                    image_part = self.image_processor.postprocess(image_part, output_type=output_type)
                    prompt_part = eval_model_llava([image_part[0]],llava_tokenizer,llava_model,llava_image_processor,llava_model_name,prompt)
                    if ngram:
                        prompt2=prompt_part
                        ...
                        divide_gram = list(ngrams(tokens2, 1))
                        divide_gram = [ngram for ngram in divide_gram if all(word not in uninformative_words for word in ngram)]
                        divide_gram = self.filter_ngrams(divide_gram)
                    ...
                    for words in negative_to_positive.keys():
                        prompt_part = prompt_part.replace(words, negative_to_positive[words])

                    prompts_part.append(prompt_part)
                ...
                if noise_decom:
                    ...
                    prompts_part=prompts_part_list
                (
                    prompt_embed,
                    negative_prompt_embed,
                    prompt_attn_mask,
                    negative_prompt_attn_mask,
                ) = self.encode_prompt(
                    prompt=prompts_part,
                    prompt_2=prompt_2,
                    device=device,
                    num_images_per_prompt=num_images_per_prompt,
                    do_classifier_free_guidance=do_classifier_free_guidance,
                    negative_prompt=[negative_prompt]*len(prompts_part),
                    negative_prompt_2=negative_prompt_2,
                    prompt_embeds=None,
                    negative_prompt_embeds=None,
                    pooled_prompt_embeds=None,
                ...
                prompt_attn_masks = []
                if do_classifier_free_guidance:
                    for i in range(prompt_embed.shape[0]):
                        prompt_embed_i = torch.cat([negative_prompt_embed[i].unsqueeze(0), prompt_embed[i].unsqueeze(0)], dim=0)
                        prompt_attn_mask_i = torch.cat([negative_prompt_attn_mask[i].unsqueeze(0), prompt_attn_mask[i].unsqueeze(0)], dim=0)
                        prompts_embed.append(prompt_embed_i)
                        prompt_attn_masks.append(prompt_attn_mask_i)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    if i>early_stop:                           
                        ...
                        views,_,_ = self.get_views(current_height, current_width, stride=stride, window_size=self.unet.config.sample_size, random_jitter=True)
                        ...
                        for j, batch_view in enumerate(views_batch):
                            ...
                            if noise_decom:
                                ...
                                for c in range(vb_size):
                                    for view_fn in views_fact:
                                        viewed_noisy_images.append(view_fn.view(latent_model_input[c]))
                                latent_model_input = torch.stack(viewed_noisy_images)
                            ...
                            if do_classifier_free_guidance:
                                ...
                                if noise_decom:
                                    ...
                                    noise_pred_uncond = torch.stack(inverted_preds)#([18, 4, 128, 128])

                                    # Invert the conditional estimates
                                    inverted_preds = []
                                    for i, (pred, view_fact) in enumerate(zip(noise_pred_text, views_fact * vb_size)):
                                        inverted_pred = view_fact.inverse_view(pred)
                                        inverted_preds.append(inverted_pred)
                                    ...
                                    if reduction == 'mean':
                                        noise_pred = noise_pred.mean(1)#([1, 3, 64, 64])
                                    elif reduction == 'sum':
                                        noise_pred = noise_pred.sum(1)#([1, 3, 64, 64])
                                    elif reduction == 'low':
                                        noise_pred = noise_pred[:, 0,:,:,:]#([1, 3, 64, 64])
                                    elif reduction == 'high':
                                        noise_pred = noise_pred[:, 1,:,:,:]#([1, 3, 64, 64])
                                    else:
    ...
    #########################################################################################################################################
                ...
                if not output_type == "latent":
                    ...
                    if multi_decoder:
                        if scale:
                            image = self.sync_tiled_decode_scale(latents, current_height, current_width)
                        else:
                            image = self.tiled_decode(latents, current_height, current_width,seed=seed)
                    else:
                        image = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
                ...
                if not output_type == "latent":
                    image = self.image_processor.postprocess(image, output_type=output_type)
                    image[0].save(image_save_path +  "/img_{}.jpg".format((current_scale_num) * 1024))
                    if show_image:
                        plt.figure(figsize=(10, 10))
                        plt.imshow(image[0])
                        plt.axis('off')  # Turn off axis numbers and ticks
